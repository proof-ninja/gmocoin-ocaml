...
type pagination = { currentPage : int; count : int } [@@deriving yojson]
...
let numeric_of_yojson json =
  ...
  | `String s -> (
      ...
      with _ -> Error (!%"numeric_of_yojson: invalid numeric string '%s'" s))
...
let numeric_to_yojson (n : numeric) : Yojson.Safe.t =
