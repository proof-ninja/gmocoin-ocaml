...
let test_side_roundtrip () =
  Alcotest.(check string) "BUY" "BUY" (Common.string_of_side (Common.side_of_string "BUY"));
  Alcotest.(check string) "SELL" "SELL" (Common.string_of_side (Common.side_of_string "SELL"));
  Alcotest.check_raises "invalid side"
...
let test_executions () =
  ...
  Alcotest.(check (option int)) "positionId" (Some 1234567) (List.hd ex.list).positionId
    ...
    [
      ...
      ( "PrivateApi",
        [
          ...
          Alcotest.test_case "orders optional fields" `Quick test_orders_optional_fields;
